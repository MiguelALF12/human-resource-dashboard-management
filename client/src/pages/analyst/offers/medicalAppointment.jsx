...
import { removeAplication, deleteRestOfAplications } from '../../../api/aplicaciones';

const MedicalInterviewAppointment = (props) => {
    ...
    const onSubmit = () => {
        ...
        if (disableCheck[1]) { //Si usuario es seleccionado (check a la caja Sí)
            ...
            createSeleccionado(selectedAplicant).then((data) => {
                if (data.selectionExists === true) {
                    alert("El aplicante ya ha sido seleccionado!")
                } else {
                ...
                return deleteRestOfAplications(aplicantId, selectedAplicant);
            }).then((data) => console.log(data)).catch(err => {
                console.warn(err);
            });
        } else if (disableCheck[0]) {
            // eliminar la aplicación del aplicante
            removeAplication(aplicationId).then((data) => {
                alert("Aplicación eliminada con exito!!");
            }).catch(err => console.warn(err));
            refreshPage();
        } else {
            alert("Debe seleccionar una casilla. (Si) para pasar de fase, (No) para remover la aplicación.");
        }
    }
        ...
        <Modal show={props.show} onHide={props.close} size="lg">
            ...
            <Form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <Row>
                        <Col xs={12} md={12} lg={12} className="px-4">
                            ...
                            <div className="d-flex flex-row justify-content-around px-3 pb-4">
                                ...
                                <div>
                                    <Calendar value={dateValue} onChange={handleDate} />
                                </div>
                            </div>
                ...
                <Modal.Footer>
                    ...
                    <Button id="sendEmail" type="submit" variant="primary">
                        Aceptar
                    </Button>
                </Modal.Footer>
            </Form>
...
export default MedicalInterviewAppointment;